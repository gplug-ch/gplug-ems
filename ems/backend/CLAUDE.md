...
| `configservice.be` | `GET/POST /api/config`: serves `site.json` verbatim and writes it back (temp file + copy over, then `site.load_config()` with rollback). Compiled per POST by the `main.be` stub (`fsx.load_transient`) and dropped afterwards — never resident, no driver of its own (issue #27); the stub serves GET itself. Since spec 011 step 2 it validates nothing beyond «is this a JSON object» — the browser owns the field rules (`einstellungen.js` `validateDocument`). |
| `modbusservice.be` | `GET /api/modbus/read` / `POST /api/modbus/write` — manual register read/write for the Einstellungen «Modbus» test panel (issue #20). By free parameters (`url`, `unit`, `function`, `register`, `dtype`, `swap_words`, `scale`, `dimension`) or by a configured item (`id` + `field` = `power`/`soc`/`energy`/`state`/`write`). Writes: holding registers only (FC 6/16; FC 5 only via a load's coil `write` block), POST only, each logged at Warn. Answers 200 `{value, raw}` / `{ok}` or `{exception}`, 400 bad params, 404 unknown id, 502 transport error (the `error` text names it: `connect to <url> failed …`, `no response within 1500 ms`, `short response`, …; a failed connect or a silent host also carries `reason` `connect`/`timeout` + `ms`, which the panel translates, issue #30). Manual ops pass `"manual": true` to `modbustcp`, which then ignores the `nethost` backoff (a panel read of a host whose poll just failed used to answer `connect failed` without trying) and never adds to it; a success still clears it. They also wait only `MANUAL_READ_BUDGET_MS` (1500) for the reply instead of the poll's `READ_BUDGET_MS` (3000) — the exchange blocks the shared Berry thread inside the HTTP handler (issue #30). Compiled per request by a `main.be` stub (`fsx.load_transient`, like `configservice`) and dropped afterwards; uses the boot-registered `modbustcp` (`site.registered_integration`) or a transient copy — never `import`s it (issue #27). Runs inline, one op. Pure `read(args)` / `write(body)` are unit-tested in `tests/modbus_write/test_modbusservice.be`. |
...
| `fsx.be` | Filesystem seam (Berry CLI `os` vs Tasmota `path`: `remove`, `listdir`, `append_line` — the store's only write path), `load_transient(name)` (compiles a module's source wrapped in `do … end` — so its names stay chunk-local, not globals — without touching the import cache; issue #27) plus the bucket-file helpers of `store.be`: `split_prefix`, `dayno`, `list_daynos`, `sort_ints`, `close_q` (issue #9). |
