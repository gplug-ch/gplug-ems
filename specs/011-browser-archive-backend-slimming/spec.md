...
**Depends on:** `001-energy-data-and-storage`, `004-verlauf-history`, `005-vzev-community`,
`006-einstellungen`, `009-vzev-betrieb-transparenz`
...
Berry bytecode source. `vzev.be` (26 KB), `store.be` (15 KB) and `webservice.be` (11 KB)
account for more than half. A code-verified analysis (2026-09-05) found three classes of
...
2. **Duplicated logic** — the browser already implements it: config validation
   (`einstellungen.js` `validate*` vs `configservice.validate`), the vZEV allocation
   (`lib/vzev.js` `allocate()` vs `vzev.allocate`).
3. **Roll-ups and late corrections** — day/month records (`store.be` FR-104) and the
   device's own vZEV share written back into sealed slots (`store.set_vzev`, the line-wise
   rewrite path). These exist only because the browser used to see just the last 4 days of
   15-min slots.

Class 3 is unlocked by a **browser-side archive**: the browser persists every 15-min record
and every raw peer slot it has fetched in **IndexedDB**, syncs incrementally on each visit,
and derives day/month roll-ups, vZEV shares, costs and quarterly billing from its own
history. The device shrinks to a **short raw buffer** (15-min buckets + peer-slot buckets,
append-only), retention lengthened from 4 to 30 days so a browser that visits at least once
...
gain — quarterly vZEV billing (spec 005 UC-504) computed over the *whole* quarter instead of
the last 4 days.
...
| A2 | `store.be` late vZEV corrections: `set_vzev`, `_rewrite_bucket`, `_rewrite_seal`, `_flush_pending`, `_find_line`, `_prune_pv15_for_day`, `pv15/pd1d/pd1mo` overlays in the cursor; `vzev.be` `_try_allocate`, `_slot_inputs`, `_record_slot_input`, `pending` map + grace-timeout branch of `tick`, `_store_set_vzev`, `allocate` | ~6–7 KB | `lib/vzev.js` `allocate()` / `flows15m()` joined to the archived 15-min records |
...
| A7 | `vzev.be` `_on_ann` conflict detection + `conflict` field; `_member_state` enrichment (`discovered`, `last_seen`) | ~0.7 KB | frontend already fetches `/api/vzev/members` and `/api/vzev/discovered` separately and joins by id |
...
append, the vZEV UDP protocol (`ann`/`slot`/`req`), member registry, peer-slot buckets,
the streaming response buffers.
...
months as before — and now with the HT/NT split (spec 009) on every past month, because the
browser holds 15-min resolution for the whole period.

### UC-1102: Quarterly settlement over the whole quarter

Persona C (producer) opens Abrechnung «Q3 2026». The browser has archived every peer slot
`/api/vzev/raw` served since spring. `buildBilling()` runs over ~8 600 slots instead of the
384 the device could keep. The quality summary (FR-906) reports true completeness for the
quarter.
...
- **FR-1101** New pure module `lib/archive.js` wrapping **IndexedDB** (never `localStorage`:
  ...
  | `vz15` | `[siteId, memberId, ts]` | `{imp, exp}` (raw peer slot; last write wins, mirrors device dedupe) |
  | `meta` | `siteId` | `{lastE15Ts, lastVzTs, firstE15Ts, gaps:[[from,to],…], syncedAt}` |
...
- **FR-1103** Incremental sync `archive.sync(api)`: on every page mount and every 15 min
  while a page is open, fetch `GET /api/energy?res=15m&count=<cap>&from=<lastE15Ts+900>` and
  `GET /api/vzev/raw`, upsert into `e15`/`vz15`, update `meta`. First sync on an empty
...
- **FR-1105** Derived series come from the archive only:
  ...
  - vZEV share per 15-min record (`vzev_in_wh`/`vzev_out_wh`, consumed by `aggregate.js`,
    `insights.js`, `verlauf.js`) is computed in the browser: `flows15m()` over the archived
    peer slots (own site + members) gives the per-slot allocation; the own share is joined
    onto the `e15` record by `ts` before aggregation. The device no longer writes these two
    fields (they are served as `0`/absent — see FR-1121).
  - Abrechnung (spec 005 UC-504, spec 009) runs `buildBilling()`/`quality()` over the
    archived peer slots for the quarter instead of `/api/vzev/raw` alone.
...
- **FR-1107** Coverage indicator (Einstellungen › «Daten» tab, new): site id, archived range
  ...
  the Verlauf header; a Verlauf/Abrechnung period that touches a gap is marked
  «provisorisch» (spec 009 glossary) with the gap range in the tooltip.
...
- **FR-1110** Export / Import: extend the existing CSV export (spec 004 FR-407) with a
  **full-archive** export (`e15` + `vz15`, one file, header row carries `siteId` and format
...
### Backend — retention & buffer (`store.be`, `vzev.be`)

- **FR-1120** `store.KEEP_DAYS` 4 → **30**; `vzev.VZ_KEEP_DAYS` 4 → **30**. Bucket file
  layout unchanged (`/e15_<dayno>`, `/vz_<san>_<dayno>`). The `_on_slot` sanity bound
  (`ts < now − 4·86400`) widens to the new retention.
- **FR-1121** `store.be` keeps **only** the 15-min path: `push_15m` (append one line),
  ...
  `<delta>,<imp>,<exp>,<pv>\n` — the optional `,<vin>,<vout>` tail is no longer written;
  the parser still accepts 6-field lines so existing buckets load (they are read as
  `vin=vout=0` and ignored). `/e1d`, `/e1mo`, `*.tmp` and the legacy `/energy.json` are
...
- **FR-1122** `GET /api/energy`: `res` accepts only `15m` (any other value → HTTP 400
  ...
  `{"ts","imp_wh","exp_wh","pv_wh","partial"?}` — `vzev_in_wh`/`vzev_out_wh` are no longer
  present. `count` default 96, cap `KEEP_DAYS·96`; `from`/`to` filters unchanged and are
  what the incremental sync relies on.
- **FR-1123** `vzev.be` keeps: registry (+ `info`), discovery, `ann`/`slot`/`req` protocol,
  peer-slot buckets, `/api/vzev/members|discovered|info|raw`. Removed: `allocate`,
  `_try_allocate`, `_slot_inputs`, `_record_slot_input`, the `pending` map and its
  grace-timeout branch in `tick`, `_store_set_vzev`, the `conflict` flag, the
  `discovered`/`last_seen` enrichment in `_member_state` (A2, A7). `announce_slot` becomes:
  store own slot → multicast. `_producer_id` stays (needed by `/api/vzev/raw`).
- **FR-1124** STORAGE.md flash budget updated. From the measured sizes in STORAGE.md §4
  (384 six-field slots = 10 296 B → ≈ 22 B per four-field line; 384 peer lines = 4 497 B
  → ≈ 12 B/line): 30 days of own 15-min lines ≈ 2 880 × 22 B ≈ **63 KB**; peer buckets ≈
  2 880 × 12 B ≈ **34 KB per member**, 3 members ≈ **101 KB**. Against the 160 KB `.tapp`
  reserve that gives 164 + 63 + 101 + 3 ≈ **331 KB — over the 320 KB partition**. The
  retention is therefore a **build-time constant with a documented ceiling**: default
  `KEEP_DAYS = 30` for the own series and `VZ_KEEP_DAYS = 14` for peer slots (3 members ≈
  47 KB → ≈ 277 KB total, ≈ 50 KB headroom). Implementer MUST recompute the STORAGE.md table
...
- **FR-1128** `main.be` `_mem_mark`/`_mem_dump` and the marks list are deleted (A8).
  `_load_integrations`, the lazy vzev/configservice stubs and the boot gate are unchanged.
- **FR-1129** Docs: `CLAUDE.md` (root + backend + frontend), `docs/features.md`, spec 001
  and 005 «Implementation note» blocks get a one-paragraph pointer to this spec for every
...
- **NFR-1104** Browser: a full sync of a 30-day buffer (≈ 2 880 records + peer slots) must
...
- **NFR-1106** Determinism (spec 005 NFR-503) is preserved *per archive*: two browsers with
  identical archived peer slots produce identical billing. Where archives differ (gaps), the
  quality summary (FR-906) makes the difference visible instead of silently diverging.
- **NFR-1107** Privacy (spec 005 FR-509) unchanged: the archive holds only what the device
  already served to that browser; nothing leaves the browser (C-2).

## Key Entities

- **Archive** — per `(origin, siteId)` IndexedDB database holding raw 15-min records and raw
  peer slots plus a coverage record.
...
- **Duplicate peer slot ts in `/api/vzev/raw`** (device appends corrections): the browser
  already treats the last triple as authoritative (`peerSlot()`); the archive upsert must
  iterate the ring in order so the same rule holds.
...
- **Two tabs syncing concurrently**: upserts are idempotent; the `meta` write uses the
  larger `lastE15Ts`.
- **Pre-spec device (v1.0.9) with new frontend**: `/api/energy` still returns the vZEV
  fields and accepts `res=1d`; the frontend ignores the fields (recomputes) and never asks
  for `1d/1mo`. Works.
...
- Moving `ems.be`, `meter.be`, the 15-min accumulation or the UDP protocol to the browser.
...
- Merging `udpclient.be` into `udpdriver.be` and de-duplicating `_sort_ints/_all_digits/
  _last_slash/_close_q` into `fsx.be` (~2 KB more) — recommended follow-up, not part of
  this slice.

## Existing Code — Extend, Don't Break

- `lib/aggregate.js`, `lib/vzev.js`, `lib/insights.js` stay pure and DOM-free; `archive.js`
...
- `tests/test_store.be`, `tests/vzev/test_vzev.be`, `tests/test_webservice_energy.be`,
  `tests/test_configservice.be`, `tests/test_ems_allocation.be` are reduced to the surviving
  behaviour (the roll-up, `set_vzev`, `_try_allocate`, `validate` and `set_production_power`
...
- **Frontend (node, `npm test`)**
  ...
  - `tests/test_vzev_lib.mjs` extended: own share derived from archived peer slots equals the
    former `store.set_vzev` values for the spec 005 UC-503 numbers.
...
- **Backend (`make test`)**
  ...
  - `test_webservice_energy.be`: `res=1d` → 400; record has no vZEV keys; `from` filter.
  - `test_vzev.be`: `announce_slot` stores + sends and does not touch `store`;
    `/api/vzev/raw` unchanged shape; members list has no `discovered`/`conflict` keys.
...
- [ ] STORAGE.md budget table recomputed for the chosen `KEEP_DAYS`/`VZ_KEEP_DAYS`, ≥ 40 KB
...
- [ ] Abrechnung over a quarter uses archived slots; quality summary reports the archive's
      real coverage
...
3. **Archive** (A1–A2, FR-1101–1111, 1120–1124): frontend archive ships **one release
   ...
   stores `e15`/`vz15`/`meta` keyed by the `/site` id), incremental sync at boot + every
   15 min, gap detection, `navigator.storage.persist()`, CSV export/import, the
   Einstellungen «Daten» tab, the FR-1111 banner, and `lib/vzev.js` `ownShare()/
   withOwnShare()` replacing the device's `set_vzev` write-back. Verlauf derives **every**
   ...
   Abrechnung settles the whole quarter, the Übersicht KPI strip joins the derived share.
   Device side of 3a: `KEEP_DAYS` 4 → 30, `VZ_KEEP_DAYS` 4 → 14, the `_on_slot` sanity
   bound follows `VZ_KEEP_DAYS`, and `GET /api/energy` with `from` now pages **forward**
   from the oldest matching slot (it returned the newest `count` before, which no
   incremental sync could walk). STORAGE.md recomputed: 15m 30 d = 77 272 B, peer 14 d =
   20 024 B/member, `.tapp` reserve lowered 160 → 128 KB, headroom ≈ 50 KB.
   ...
   `test_vzev_lib.mjs` extended, Berry `test_store.be`/`test_webservice_energy.be` updated.
   ...
   `set_vzev` and the `.tmp` line-wise rewrite are gone, RAM state is `nb` alone (NFR-1103),
   and a 6-field legacy line still parses so existing buckets survive the upgrade.
   `GET /api/energy` serves `res=15m` only (400 otherwise) and records carry no vZEV fields
   (FR-1122). `vzev.be` (25 624 → 22 021 B) lost `allocate`, `_try_allocate`, `_slot_inputs`,
   `_record_slot_input`, the `pending` map + grace branch and `_store_set_vzev`;
   `announce_slot` is «store the own slot → multicast» and the module no longer imports
   `store` at all. Frontend: `verlauf.js` dropped the `res=1d|1mo` transition fallback and
   ...
   unchanged at 30 / 14 days.

   Deviations worth recording: `store.be`'s `last_ts` was dropped too (its only reader was
   `set_vzev`), and `day_start`/`month_start` went with the roll-ups — nothing outside the
   ...
   `Promise.all` destructuring, which had been feeding the device roll-up array to
   `withOwnShare()` and the archived peer slots to the roll-up prepend.
